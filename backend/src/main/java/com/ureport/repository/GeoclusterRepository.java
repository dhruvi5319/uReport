 ...
 *
 * Fix: PostgreSQL POINT has no equality operator, so GROUP BY on g.center fails.
 * Solution: Use MIN(CAST(g.center AS TEXT)) AS center_text aggregate, and remove
 * g.center from GROUP BY — only cluster_id_N and g.level remain in GROUP BY.
 */
public interface GeoclusterRepository extends JpaRepository<Geocluster, Long> {
    ...
    @Query(value = """
        ...
        JOIN tickets t ON t.id = tg.ticket_id
        WHERE (:status IS NULL OR t.status = :status)
        GROUP BY tg.cluster_id_1, g.level
        """, nativeQuery = true)
    ...
    @Query(value = """
        ...
        JOIN tickets t ON t.id = tg.ticket_id
        WHERE (:status IS NULL OR t.status = :status)
        GROUP BY tg.cluster_id_2, g.level
        """, nativeQuery = true)
    ...
    @Query(value = """
        ...
        JOIN tickets t ON t.id = tg.ticket_id
        WHERE (:status IS NULL OR t.status = :status)
        GROUP BY tg.cluster_id_3, g.level
        """, nativeQuery = true)
    ...
    @Query(value = """
        ...
        JOIN tickets t ON t.id = tg.ticket_id
        WHERE (:status IS NULL OR t.status = :status)
        GROUP BY tg.cluster_id_4, g.level
        """, nativeQuery = true)
    ...
    @Query(value = """
        ...
        JOIN tickets t ON t.id = tg.ticket_id
        WHERE (:status IS NULL OR t.status = :status)
        GROUP BY tg.cluster_id_5, g.level
        """, nativeQuery = true)
    ...
    @Query(value = """
        ...
        JOIN tickets t ON t.id = tg.ticket_id
        WHERE (:status IS NULL OR t.status = :status)
        GROUP BY tg.cluster_id_6, g.level
        """, nativeQuery = true)
